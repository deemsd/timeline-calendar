...
export async function waitForInsert(
  ...
  return await safeExecute(async () => {
    ...
    // Format the date
    const startMoment = startDate instanceof Date ? moment(startDate) : moment(startDate);
    const endMoment = endDate instanceof Date ? moment(endDate) : moment(endDate);

    const formattedContent = formatEventLine(EventContent, startMoment, endMoment, 'TASK-TODO');
...
): Promise<{file: TFile; lineNum: number}> {
