...
export function cleanContent(content: string, dates: DateInfo[], recurrenceRule?: string, blockLink?: string): string {
  ...
  // Remove end time markers (⏲ HH:MM)
  result = result.replace(/⏲\s?\d{1,2}:\d{2}(:\d{2})?/g, '');

  // Remove trailing time range (e.g. "任务名 10:00-10:30" → "任务名")
  result = result.replace(/\s\d{1,2}:\d{2}-\d{1,2}:\d{2}/g, '');
...
export function convertToEvent(
  ...
  if (isTask && !hasTimeInfo) {
    ...
    const startTime = parsedLine.startTime;
    // Set start time
    if (startTime) {
      startDate.hour(startTime.hour);
      startDate.minute(startTime.minute);
      startDate.second(startTime.second || 0);
    }
    ...
    if (hasDueDate && startTime) {
      if (!parsedLine.endTime) {
        // If no end time was specified, keep the same time as start
        endDate.hour(startTime.hour);
        endDate.minute(startTime.minute);
        endDate.second(startTime.second || 0);
      }
    }
  }
