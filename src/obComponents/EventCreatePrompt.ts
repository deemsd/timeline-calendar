...
export default class EventCreatePrompt extends Modal {
  ...
  private display() {
    ...
    const mainContentContainer: HTMLDivElement = this.contentEl.createDiv({
      cls: 'timeline-calendar-event-prompt',
    });
    ...
    this.createDateTimePicker(mainContentContainer, 'Start time', this.startDate, (date) => {
      ...
      if (this.endDate < this.startDate) {
        this.endDate = new Date(this.startDate.getTime() + 60 * 60 * 1000);
        const endDateInput = mainContentContainer.querySelector<HTMLInputElement>('#end-date-input');
        const endTimeInput = mainContentContainer.querySelector<HTMLInputElement>('#end-time-input');
  ...
  private createDateTimePicker(
    ...
    const dateTimeContainer = container.createDiv({
      cls: 'timeline-calendar-event-prompt__datetime',
    });

    new Setting(dateTimeContainer).setName(name);

    const inputContainer = dateTimeContainer.createDiv({
      cls: 'timeline-calendar-event-prompt__datetime-inputs',
    });
    ...
    dateInput.addClass('timeline-calendar-event-prompt__date-input');
    ...
    timeInput.addClass('timeline-calendar-event-prompt__time-input');
  ...
  protected createInputField(container: HTMLElement, placeholder?: string, value?: string) {
    ...
    textComponent.inputEl.addClass('timeline-calendar-event-prompt__content');
    textComponent
  ...
  private createButtonBar(mainContentContainer: HTMLDivElement) {
    const buttonBarContainer: HTMLDivElement = mainContentContainer.createDiv({
      cls: 'timeline-calendar-event-prompt__button-bar',
    });
    this.createButton(buttonBarContainer, 'Create', this.submitClickCallback).setCta();
  ...
  private submit() {
    if (!this.content) {
      ...
      const errorDiv = this.contentEl.createDiv({
        cls: 'timeline-calendar-event-prompt__error',
      });
