...
import {moment, Menu, Notice} from 'obsidian';
...
interface EventProps<TEvent = Event> {
  ...
  localizer: unknown; // Changed from optional to required
...
const EventComponent: React.FC<EventProps<Event>> = ({
  ...
  // Check if event has a task type
  ...
  const isTaskEvent = !!event.eventType?.startsWith('TASK-');
  const isTaskChecked = isTaskEvent && event.eventType !== 'TASK-TODO';

  const stopCheckboxPropagation = (e: React.SyntheticEvent) => {
    e.stopPropagation();
  };

  const handleTaskCheckboxChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    e.stopPropagation();
    ...
    const nextType = event.eventType === 'TASK-TODO' ? 'TASK-DONE' : 'TASK-TODO';

    try {
      await eventService.updateEvent(event.id, event.title, nextType, event.start, event.end);
    } catch (error) {
  ...
  const handleContextMenu = (e: React.MouseEvent) => {
      ...
      item
        ...
        .onClick(async () => {
          try {
            ...
            await eventService.createEvent(event.title, event.start, event.end);
          } catch (error) {
        ...
        item
          ...
          .onClick(async () => {
            ...
            try {
              ...
              if (!updatedEvent) {
                console.error('Failed to change event type');
              }
            } catch (error) {
    ...
    menu.addItem((item) => {
      item
        ...
        .onClick(async () => {
          try {
            const deleted = await eventService.deleteEventById(event.id);
            if (!deleted) {
              new Notice('删除失败：未找到原始任务行');
      ...
      {!isDefaultType && (
        <span className="event-type-marker">
          <input
            ...
            onChange={(e) => void handleTaskCheckboxChange(e)}
          ></input>
        </span>
      )}
